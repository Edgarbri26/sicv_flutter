...
class MovementService {
  ...
  Future<List<MovementSummaryModel>> getAll() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception(
          'Error del servidor ($msg) - Código: ${response.statusCode}',
        );
      }
    } catch (e) {
      // CAMBIO 6: IMPORTANTE
      // No ocultes el error original. Imprímelo y lánzalo para saber si es parseo o red.
      debugPrint("Error en getAll Movements: $e");
  ...
  Future<MovementModel> getMovementById(int id) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final Map<String, dynamic> movementJson =
  ...
  Future<MovementModel> createAdjustment(
    ...
    final isPositive =
        movement.type == 'Ajuste Positivo' || movement.type == 'Entrada';
    ...
    try {
      ...
      if (response.statusCode == 201 || response.statusCode == 200) {
        ...
        throw Exception(
          errorBody['message'] ??
              'Error al crear ajuste (${response.statusCode})',
        );
      }
    } catch (e) {
